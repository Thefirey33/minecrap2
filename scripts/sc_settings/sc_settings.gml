...
function sc_settings(screen_group, gui_handler){
    ...
    var _is_controller = new tte_checkbox(
        gui_handler.x,
        _fullscreen_button.y + _fullscreen_button.get_measurements()[1] + OUTLINE_MARGIN * 2,
        tte_get_localization(
            global.CURRENT_LANGUAGE,
            "gamepad"
        ),
    ...
    var _keybinding = new tte_gui_text(
        gui_handler.x, 
        _is_controller.y + _is_controller.get_measurements()[1] + OUTLINE_MARGIN * 2, 
        tte_get_localization(
            global.CURRENT_LANGUAGE,
            "keybinding"
        ),
    ...
    for (var i = 0; i < array_length(_assignmentNames); i++) {
        ...
        var _keybinding_button = new tte_selectable_button_text(
            gui_handler.x,
            _lastValue.y + _lastValue.get_measurements()[1] + OUTLINE_MARGIN * 2,
