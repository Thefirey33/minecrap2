...
var _current_use = global.GAMEPAD_SYSTEM.gamepad_configurations[$ "currently_using"]
...
for (var _x = 0; _x < ds_grid_width(_base_keyboard_layout); _x++) {
    for (var _y = 0; _y < ds_grid_height(_base_keyboard_layout); _y++) {
        ...
        _val = tte_get_gamepad_held(
            _current_use,
            global.GAMEPAD_SYSTEM.gamepad_configurations[$ "player"][$ "upshift"]
        ) ? _val : string_lower(_val)
...
if _cooldown <= 0 {
    if tte_get_gamepad_pressed(_current_use, global.GAMEPAD_SYSTEM.gamepad_configurations.player.interact) {
        self._isActive = false
    }
    if tte_get_gamepad_pressed(_current_use, global.GAMEPAD_SYSTEM.gamepad_configurations.player.backspace) {
        keyboard_key_press(vk_backspace)
    }
    if tte_get_gamepad_pressed(_current_use, global.GAMEPAD_SYSTEM.gamepad_configurations.player.space) {
